import React, { useState } from "react";
import { useAppDispatch } from "../../../app/hooks";
import { useHandleResponseError } from "../../../hooks/useHandleResponseError";
import { QuestionModel } from "../../../models/question";
import AddReadingQuestion from "./AddReadingQuestion";
import AddImageParagraphReading from "./AddImageParagraphReading";
import { TestDetailModel } from "../../../models/test";
import {
  showConfirmModal,
  showSuccessModal,
} from "../../../components/modals/CommonModals";
import { setLoading } from "../../../redux/globalSlice";
import questionApi from "../../../api/questionApi";

interface ReadingPartProps {
  testId: string;
  questions: QuestionModel[];
  afterDelete: () => void;
  test: TestDetailModel;
}

const ReadingPart: React.FunctionComponent<ReadingPartProps> = ({
  testId,
  questions,
  afterDelete,
  test,
}) => {
  const dispatch = useAppDispatch();
  const handlResponseError = useHandleResponseError();
  const [isOpen, setIsOpen] = useState<boolean>(false);
  const [isOpenResource, setIsOpenResource] = useState<boolean>(false);
  const [question, setQuestion] = useState<QuestionModel | null>(null);

  const handleDeleteQuestion = async (id: number) => {
    dispatch(setLoading("ADD"));
    const { ok, error } = await questionApi.deleteQuestion(id);
    dispatch(setLoading("REMOVE"));

    if (ok) {
      showSuccessModal({
        content: "Delete question successfully.",
        title: "Notification",
        onOk: () => {},
      });
    } else {
      handlResponseError(error);
    }
    afterDelete();
  };

  const handleDeleteClick = (id: number) => {
    showConfirmModal({
      title: "Notification",
      content: "Are you sure to delete this question?",
      onOk: () => handleDeleteQuestion(id),
      maskClosable: false,
    });
  };

  return (
    <div className="test-part" style={{ marginTop: "30px" }}>
      <div className="test-part__header">
        <h3>Reading Part</h3>
        <div
          style={{
            display: "flex",
            flexDirection: "row",
            alignItems: "center",
            gap: "10px",
          }}
        >
          <button
            onClick={() => setIsOpenResource(true)}
            style={{ width: "fit-content", padding: "0 10px" }}
          >
            Add / Edit resource
          </button>
          <button onClick={() => setIsOpen(true)}>Add question</button>
        </div>
      </div>
      <div className="test-part__reading-resource">
        {test.image && <img src={test.image} alt="reading" />}
        {test.paragraph && (
          <p>
            <span style={{ fontWeight: 700 }}>Paragraph: </span>
            {test.paragraph}
          </p>
        )}
      </div>
      {questions.length !== 0 ? (
        <div className="test-part__questions">
          {questions.map((q, index) => (
            <div className="question" key={`listening-question-${index}`}>
              <p style={{ fontSize: "16px" }}>
                <span style={{ fontWeight: 600 }}>{`Question ${
                  index + 1
                }: `}</span>
                {q.question}
              </p>
              <div className="answers">
                {q.answers.map((a, i) => (
                  <p key={`listening-question-${index}-answer-${i}`}>
                    <span>{`Answer ${i + 1}: `}</span>
                    {a.content}
                    <span className={`${a.isResult ? "result" : ""}`}></span>
                  </p>
                ))}
              </div>
              <div className="question__footer">
                <button onClick={() => setQuestion(q)}>Edit</button>
                <button onClick={() => handleDeleteClick(q.id)}>Delete</button>
              </div>
            </div>
          ))}
        </div>
      ) : (
        <div className="empty_data">
          <img
            alt="empty"
            src="data:image/png;base64,"
          />
          <span>No data</span>
        </div>
      )}

      <AddReadingQuestion
        testId={testId}
        isOpen={isOpen}
        onClose={() => setIsOpen(false)}
        afterAdd={afterDelete}
      />
      <AddImageParagraphReading
        testId={testId}
        isOpen={isOpenResource}
        onClose={() => setIsOpenResource(false)}
        afterAdd={afterDelete}
        paragraph={test.paragraph}
      />
    </div>
  );
};

export default ReadingPart;
